...
```bash
python "~/.claude/scripts/session-list.py" --workspace current --json
```

如果用户要求「所有会话」「全部工作区」「all sessions」：

```bash
python "~/.claude/scripts/session-list.py" --workspace all --json
```
...
```bash
python "~/.claude/scripts/session-rename.py" --confirm-latest
```
...
```bash
python "~/.claude/scripts/session-rename.py" --id "XXX" --name "YYY"
```
...
```bash
python "~/.claude/scripts/session-clean.py" --list
```
...
```bash
python "~/.claude/scripts/session-clean.py" --delete <id1> <id2> ...
```
